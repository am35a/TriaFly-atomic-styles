<script>
    // import successkid from 'images/successkid.jpg';
</script>

<style>
    /* Here is empty, all into global.css */
</style>

<svelte:head>
    <title>TriaFly UI Library - CSS/JS framework for TriaFly BI system</title>
</svelte:head>

<div class="d-grid g-4">
    <div>
        <img class="d-inline-flex" src="images/logo-icon.svg" alt="TriaFly" style="width: 16rem;">
        <div class="h1" role="heading" aria-level="1">TriaFly UI Library v4.28</div>
    </div>
    <div>
        <strong>This tool make design easier to create responsive and high-loaded applications.</strong>
        <p>Easy to design and multiple settings of responsive Bootstrap interfaces makes it a promising toolkit with lots of diverse components.</p>
    </div>
</div>
